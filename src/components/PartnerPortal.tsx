...
export default function PartnerPortal({ appVersion }: { appVersion?: string }) {
  const { orders, products, loading: dataLoading } = useData();
  ...
  const handleAddProduct = async (e: React.FormEvent) => {
    ...
    try {
      await addProductToFirestore(newProductForm);
  ...
  const loadingOrders = dataLoading;
  const loadingProducts = dataLoading;

  const refreshProducts = async () => {
    // Products refresh automatically via DataContext
  };
  ...
  return (
    <div className="min-h-screen bg-slate-50 flex flex-col font-sans">
        ...
        <div className="max-w-7xl mx-auto px-4 py-4 flex flex-col sm:flex-row justify-between items-center gap-3">
          <div className="flex items-center gap-2.5">
            ...
            <div>
              ...
              <p className="text-[10px] text-slate-300 font-semibold uppercase hidden sm:block">Fulfillment Desk & Real-time Rider Console</p>
            </div>
          </div>
      ...
      {/* Main Container */}
      <main className="flex-1 max-w-7xl w-full mx-auto p-4 grid grid-cols-1 lg:grid-cols-3 gap-6 lg:overflow-hidden overflow-y-auto">
        {activeTab === 'orders' ? (
          <>
            {/* Orders Feed */}
            ...
            <div className={`${
              selectedOrder 
                ? 'fixed inset-0 z-50 bg-slate-900/40 backdrop-blur-xs flex items-center justify-center p-4 lg:relative lg:inset-auto lg:bg-transparent lg:backdrop-blur-none lg:z-auto lg:p-0 lg:col-span-1 lg:h-[calc(100vh-140px)] lg:sticky lg:top-4'
                : 'hidden lg:flex lg:col-span-1 bg-white border border-slate-200 rounded-3xl p-5 shadow-sm flex flex-col justify-center items-center text-center overflow-hidden h-[calc(100vh-140px)] sticky top-4'
            }`}>
              {selectedOrder ? (
                <div className="bg-white border border-slate-200 rounded-3xl p-5 shadow-2xl flex flex-col justify-between overflow-hidden h-full max-h-[90vh] w-full max-w-lg lg:max-h-none lg:shadow-sm lg:rounded-3xl lg:h-full lg:max-w-none text-left">
