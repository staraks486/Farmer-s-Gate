...
import { Store, Sale, Purchase, InventoryItem, Requirement, CustomerOrder, CpanelSettings, AppNotification, StaffMember, AttendanceRecord, AccountEntry, AttendancePunch, MasterCrop, OfferPromo, MilkCustomer } from '../types';
...
import { subscribeToMilkCustomers, saveMilkCustomerToDb } from '../lib/farmersGateDb';
...
export default function StoreManager({
  ...
  const [activeSubTab, setActiveSubTab] = useState<'sale' | 'sales-history' | 'purchase' | 'inventory' | 'requirements' | 'info' | 'qr-code' | 'attendance' | 'expenses' | 'report' | 'stock-transfer' | 'stock-waste' | 'milk-users'>('sale');

  // Smart Device lightweight layout toggle
  const [smartDeviceMode, setSmartDeviceMode] = useState<boolean>(false);

  const toggleSmartDeviceMode = () => {
    // Deprecated to minimize smart device optimization and focus on rich billing screen
    setSmartDeviceMode(false);
  };

  // Real-time Milk Customers state
  ...
  useEffect(() => {
    const unsubscribe = subscribeToMilkCustomers((dbCustomers) => {
      if (dbCustomers && dbCustomers.length > 0) {
        const sorted = [...dbCustomers].sort((a, b) => {
          return new Date(b.createdAt || '').getTime() - new Date(a.createdAt || '').getTime();
        });
        localStorage.setItem('fg_milk_customers', JSON.stringify(sorted));
        setMilkCustomers(sorted);
      }
    });
    return () => unsubscribe();
  }, []);
  ...
  const [printPreviewBill, setPrintPreviewBill] = useState<any | null>(null);
  ...
  // Stock Order Checklist states
  const [isChecklistMode, setIsChecklistMode] = useState<boolean>(true);
  const [checklistSearchQuery, setChecklistSearchQuery] = useState<string>('');
  const [checklistQuantities, setChecklistQuantities] = useState<Record<string, string>>({});
  const [checklistNotes, setChecklistNotes] = useState<Record<string, string>>({});
  const [customRequirementName, setCustomRequirementName] = useState<string>('');
  const [customRequirementQty, setCustomRequirementQty] = useState<string>('');
  ...
  const [whatsappPhone, setWhatsappPhone] = useState(() => {
    try {
      return localStorage.getItem('fg_last_pos_whatsapp_phone') || '';
    } catch {
      return '';
    }
  });
  ...
  useEffect(() => {
    if (whatsappPhone) {
      localStorage.setItem('fg_last_pos_whatsapp_phone', whatsappPhone);
    }
  }, [whatsappPhone]);
  ...
  const [saveCustomerBill, setSaveCustomerBill] = useState(true);
  const [syncCustomerToDirectory, setSyncCustomerToDirectory] = useState(true);
  ...
  const [billingTabs, setBillingTabs] = useState<{
    ...
    { id: 'bill-1', label: 'Bill 1', cart: {}, customerName: '', whatsappPhone: '', customerEmail: '', customerAddress: '', customerNotes: '', salespersonName: '' },
    { id: 'bill-2', label: 'Bill 2', cart: {}, customerName: '', whatsappPhone: '', customerEmail: '', customerAddress: '', customerNotes: '', salespersonName: '' },
    { id: 'bill-3', label: 'Bill 3', cart: {}, customerName: '', whatsappPhone: '', customerEmail: '', customerAddress: '', customerNotes: '', salespersonName: '' }
  ]);
  ...
  const handleSwitchBillingTab = (targetTabId: string) => {
    setBillingTabs(prev => {
      const updated = prev.map(t => {
        if (t.id === activeBillingTabId) {
          return {
            ...
            customerNotes: posCustomerNotes,
      ...
      if (targetTab) {
        ...
        setPosCustomerEmail(targetTab.customerEmail || '');
        setPosCustomerAddress(targetTab.customerAddress || '');
        setPosCustomerNotes(targetTab.customerNotes || '');
        setSalespersonName(targetTab.salespersonName || '');
  ...
  const handleDeleteBillingTab = (tabId: string, e: React.MouseEvent) => {
    ...
    setBillingTabs(prev => {
      ...
      if (activeBillingTabId === tabId) {
        ...
        setSalespersonName(nextActive.salespersonName || '');
  ...
  const [completedBill, setCompletedBill] = useState<{
    ...
    try {
      ...
      if (saved) {
        ...
        if (parsed.storeId === store.id) return parsed;
      }
      return null;
    } catch {
      return null;
  ...
  const handlePosCheckoutSubmit = async (e: React.FormEvent) => {
    e.preventDefault();
    ...
    // Always save the bill inside the app's central ledger/history to satisfy "Save bill in the app"
    try {
      const savedBillsStr = localStorage.getItem('fg_bills') || '[]';
      const parsed = JSON.parse(savedBillsStr);
      // Avoid duplicating if it already exists
      if (!parsed.some((b: any) => b.id === finalBill.id)) {
    ...
    if (syncCustomerToDirectory && customerNameTrimmed) {
      try {
        const phoneClean = whatsappPhone.trim() || 'No Phone';
        const updatedCusts = [...generalCustomers];
        
        let matchIdx = -1;
        if (phoneClean !== 'No Phone') {
          matchIdx = updatedCusts.findIndex(c => c.phone === phoneClean && c.storeId === store.id);
        } else {
          matchIdx = updatedCusts.findIndex(c => c.name.toLowerCase() === customerNameTrimmed.toLowerCase() && c.storeId === store.id);
        }
        ...
        if (matchIdx > -1) {
          // Customer exists, update loyalty points and details
          updatedCusts[matchIdx] = {
            ...updatedCusts[matchIdx],
            name: customerNameTrimmed, // Keep name synchronized or updated
            phone: phoneClean,
            email: posCustomerEmail.trim() || updatedCusts[matchIdx].email || '',
            address: posCustomerAddress.trim() || updatedCusts[matchIdx].address || '',
            loyaltyPoints: (updatedCusts[matchIdx].loyaltyPoints || 0) + (saveCustomerBill ? pointsEarned : 0),
            notes: posCustomerNotes.trim() || updatedCusts[matchIdx].notes || 'Updated via POS Checkout'
          };
        } else {
          ...
          updatedCusts.push({
            ...
            phone: phoneClean,
            email: posCustomerEmail.trim(),
            address: posCustomerAddress.trim(),
            loyaltyPoints: saveCustomerBill ? pointsEarned : 0,
            createdAt: new Date().toISOString(),
            notes: posCustomerNotes.trim() || 'Registered automatically during POS checkout'
          });
        }
    ...
    // Update user logs in Milk User module
    if (selectedSubId) {
      try {
        ...
        if (savedCustomers) {
          ...
          if (customerIndex > -1) {
            ...
            // Determine quantities taken based on POS cart or user profile
            let cowTaken = 0;
            let buffaloTaken = 0;
            ...
            // Find if any cart items represent user milk and override
            ...
            const newLogEntry = {
              ...
              quantity: cowTaken + buffaloTaken,
            ...
            await saveMilkCustomerToDb(customer);
          }
        }
      } catch (err) {
        console.error("Failed to sync Milk User POS delivery log:", err);
      }
    }

    // Clear cart and states
    setPosCart({});
    setSelectedSubId(''); // Clear selected user state on checkout
    setPosCustomerName('');
    setWhatsappPhone('');
    setPosCustomerEmail('');
    setPosCustomerAddress('');
  ...
  const getWhatsAppShareUrl = () => {
    ...
    if (store.location) msg += `*Location:* ${store.location}\n`;
    msg += `*Date:* ${completedBill.date}\n`;
    msg += `*Customer:* ${completedBill.customerName || 'Retail Customer'}\n`;
    if (completedBill.salespersonName) {
      msg += `*Salesperson:* ${completedBill.salespersonName}\n`;
    }
    msg += `\n*Items Purchased:*\n`;
    ...
    // Add Digital Receipt & Download PDF Link back
  ...
  const getWhatsAppQuoteUrl = () => {
    ...
    let msg = `*ORDER ESTIMATE / QUOTE*\n`;
    msg += `*Store:* ${store.name}\n`;
    if (store.location) msg += `*Location:* ${store.location}\n`;
    msg += `*Date:* ${new Date().toLocaleString()}\n`;
    msg += `*Customer:* ${posCustomerName || 'Retail Customer'}\n`;
    msg += `\n*Cart Summary:*\n`;

    cartItems.forEach((it, idx) => {
      const unitLabel = it.unit || 'kg';
      const sub = getSubtotal(it);
      msg += `${idx + 1}. ${it.item.vegetableName} - ${it.quantity} ${unitLabel} = *₹${sub.toFixed(2)}*\n`;
    });

    const total = cartItems.reduce((acc, curr) => acc + getSubtotal(curr), 0);
    msg += `\n*ESTIMATED TOTAL: ₹${total.toFixed(2)}*\n\n`;
    msg += `_Note: This is an estimated quote. Final prices may vary at checkout._\n\n`;
    msg += `Thank you for shopping at our fresh farm store! 🌱🍎`;
  ...
  const handleSharePDF = (customBill?: any) => {
    ...
    try {
      ...
      const file = new File([pdfBlob], `FG_Receipt_${billToUse.id}.pdf`, { type: 'application/pdf' });

      if (navigator.share && navigator.canShare && navigator.canShare({ files: [file] })) {
        navigator.share({
          files: [file],
          title: `Receipt ${billToUse.id}`,
          text: `Farmer's Gate Receipt for Invoice #${billToUse.id}`
        }).catch(err => {
      ...
      alert("PDF generation triggered. If you are on mobile, please check your notifications or downloads.");
    }
  };
  ...
  const handleShareImage = (customBill?: any) => {
    const billToUse = (customBill && typeof customBill === 'object' && 'id' in customBill) ? customBill : completedBill;
    if (!billToUse) return;
    const canvas = document.createElement('canvas');
    canvas.width = 400;
    canvas.height = 160 + billToUse.items.length * 45 + 160;
    ...
    billToUse.items.forEach((it: any) => {
    ...
    ctx.textAlign = 'right';
    ctx.fillText(`INR ${billToUse.totalAmount.toFixed(2)}`, canvas.width - 25, y + 10);

    if (billToUse.paymentMode) {
      y += 18;
      ctx.fillStyle = '#64748b';
      ...
      ctx.textAlign = 'right';
      ctx.fillText(billToUse.paymentMode.toUpperCase(), canvas.width - 25, y + 10);
    }
    ...
    canvas.toBlob(async (blob) => {
      if (!blob) return;
      const file = new File([blob], `${billToUse.id}.png`, { type: 'image/png' });
      if (navigator.share && navigator.canShare && navigator.canShare({ files: [file] })) {
        try {
          await navigator.share({
            title: `Invoice ${billToUse.id}`,
            text: 'Here is your bill from Farmer\'s Gate.',
            files: [file]
          });
        } catch (error) {
          console.error('Error sharing image:', error);
          const link = document.createElement('a');
          link.download = `${billToUse.id}.png`;
  ...
  const storeSales = React.useMemo(() => sales.filter(s => s.storeId === store.id), [sales, store.id]);
  const storePurchases = React.useMemo(() => purchases.filter(p => p.storeId === store.id), [purchases, store.id]);
  const storeInventory = React.useMemo(() => {
    const raw = inventory.filter(i => i.storeId === store.id);
    const seen = new Set<string>();
    const unique: typeof raw = [];
    for (const item of raw) {
      const normalized = (item.vegetableName || "").toLowerCase().trim();
  ...
  const storeRequirements = React.useMemo(() => requirements.filter(r => r.storeId === store.id), [requirements, store.id]);
  ...
  const handleChecklistAddRequirement = (vegName: string) => {
    const qtyStr = checklistQuantities[vegName];
    const qty = parseFloat(qtyStr);
    if (isNaN(qty) || qty <= 0) return;

    // Check if there is an existing pending requirement for this item
    const existingReq = storeRequirements.find(r => r.vegetableName === vegName && r.status === 'Pending');

    if (existingReq && onUpdateRequirement) {
      onUpdateRequirement({
        ...existingReq,
        quantity: qty,
        notes: checklistNotes[vegName] || existingReq.notes
      });
    } else {
      const newReq: Requirement = {
        id: generateId('req'),
        ...
        status: 'Pending',
        priority: 'Medium',
        notes: checklistNotes[vegName] || undefined,
        createdAt: new Date().toISOString()
      };
      onAddRequirement(newReq);
    }
  ...
  const handleCustomRequirementSubmit = (e: React.FormEvent) => {
    e.preventDefault();
    if (!customRequirementName.trim() || !customRequirementQty || parseFloat(customRequirementQty) <= 0) return;

    const newReq: Requirement = {
      id: generateId('req'),
      storeId: store.id,
      vegetableName: customRequirementName.trim(),
      quantity: parseFloat(customRequirementQty),
      status: 'Pending',
      priority: 'Medium',
      createdAt: new Date().toISOString()
    };
  ...
  return (
    <div className={`space-y-4 animate-fade-in text-slate-900 ${smartDeviceMode ? 'light-device-mode density-high font-sans' : ''}`}>
      ...
      <div className="flex border-b border-slate-200 gap-1 overflow-x-auto scrollbar-none shrink-0 py-1 bg-white/50 backdrop-blur-md sticky top-0 z-10 touch-pan-x overscroll-contain">
        ...
        <button
          id="tab-milk-users"
          onClick={() => setActiveSubTab('milk-users')}
          className={`flex items-center gap-1.5 px-3 py-2 text-xs font-bold border-b-2 whitespace-nowrap transition-all active:scale-95 duration-100 cursor-pointer ${
            activeSubTab === 'milk-users'
              ? 'border-emerald-600 text-emerald-600 bg-emerald-50/20 font-black'
              : 'border-transparent text-slate-500 hover:text-slate-800 hover:bg-slate-50'
          }`}
        >
      ...
      {/* --- SUB-TAB CONTENT: Store Milk Record --- */}
      {activeSubTab === 'milk-users' && (
        <div className="bg-white rounded-3xl p-6 border border-slate-200/85 shadow-xs animate-in fade-in duration-300">
          <CustomerMilkRegistry 
            ...
            sales={sales}
            requestSecureAction={cpanelSettings?.enableDataChangeAuth ? requestSecureAction : undefined}
          />
        </div>
      )}
        ...
        <div className="space-y-6">
          {completedBill ? (
              ...
              <div className="max-w-4xl mx-auto flex flex-col md:flex-row gap-8">
                ...
                <div className="flex-1 max-w-sm mx-auto bg-gradient-to-b from-white to-slate-50/20 border border-slate-200 rounded-3xl p-6 shadow-md font-mono text-xs text-slate-800 space-y-4 relative overflow-hidden">
                  ...
                  <div className="bg-slate-50/80 rounded-xl p-2.5 border border-slate-150 text-[10px] space-y-1 text-left">
                    ...
                    {completedBill.salespersonName && (
                      <div className="flex justify-between text-slate-500 font-bold text-[9px] mt-1 pt-1 border-t border-slate-100">
                        <span>👤 SALESPERSON:</span>
                        <span className="font-extrabold uppercase">{completedBill.salespersonName}</span>
                      </div>
                    )}
                ...
                <div className="flex-1 flex flex-col justify-between space-y-6">
                  <div className="bg-white border border-slate-200 rounded-2xl p-5 space-y-4 shadow-sm">
                    ...
                    <div className="border-t border-slate-100 pt-4 space-y-3">
                      ...
                      <div className="flex gap-2">
                        ...
                        <a
                          ...
                          className="bg-[#25D366] text-white hover:bg-[#128C7E] px-4 py-2.5 rounded-xl text-xs font-extrabold transition-all shadow-sm shrink-0 cursor-pointer flex items-center justify-center gap-2"
                        >
                    ...
                    <div className="border-t border-slate-100 pt-4 space-y-2 text-left">
                      <label className="block text-[10px] font-black uppercase tracking-wider text-slate-500">
                        Digital Bill / Download PDF Link
                      </label>
                      <div className="flex gap-2">
                        <input
                          type="text"
                          readOnly
                          value={`${window.location.origin}/#invoice?id=${completedBill.id}`}
                          onClick={(e) => (e.target as HTMLInputElement).select()}
                          className="flex-1 rounded-xl border border-slate-200 py-2.5 px-3.5 text-[11px] font-mono font-bold text-slate-600 bg-slate-50 cursor-pointer select-all focus:outline-none"
                        />
                        <button
                          onClick={() => {
                            navigator.clipboard.writeText(`${window.location.origin}/#invoice?id=${completedBill.id}`);
                            alert("Digital bill URL copied to clipboard!");
                          }}
                          className="bg-emerald-600 hover:bg-emerald-700 text-white px-4 py-2.5 rounded-xl text-xs font-extrabold transition-all shadow-sm shrink-0 cursor-pointer flex items-center justify-center gap-1"
                        >
                          Copy Link
                        </button>
                      </div>
                      <div className="flex justify-between items-center text-[10px] font-black">
                        <a
                          href={`${window.location.origin}/#invoice?id=${completedBill.id}`}
                          target="_blank"
                          rel="noreferrer"
                        ...
                        <span className="text-slate-400">PDF download option inside link</span>
                      </div>
                    </div>
                    
                    <div className="flex gap-2">
                      <button
                        onClick={() => handleShareImage(completedBill)}
                        className="flex-1 bg-white border border-slate-200 hover:bg-slate-50 text-slate-700 px-4 py-2.5 rounded-xl text-xs font-extrabold transition-all shadow-sm cursor-pointer flex items-center justify-center gap-1"
                      >
                        📄 Share as Image
                      </button>
                      <button
                        onClick={() => handleSharePDF(completedBill)}
                        className="flex-1 bg-white border border-slate-200 hover:bg-slate-50 text-slate-700 px-4 py-2.5 rounded-xl text-xs font-extrabold transition-all shadow-sm cursor-pointer flex items-center justify-center gap-1"
                      >
                        📕 Share as PDF
                      </button>
                    ...
                    <button
                      type="button"
                      onClick={() => setPrintPreviewBill(completedBill)}
                      className="w-full bg-slate-100 hover:bg-slate-200 text-slate-800 border border-slate-200 px-4 py-2.5 rounded-xl text-xs font-extrabold transition-all shadow-sm cursor-pointer flex items-center justify-center gap-1.5"
                    >
                      🖨️ Print Preview
                    </button>

                    <div className="bg-slate-100 p-4 rounded-2xl border border-slate-200 text-center">
                      <span className="block text-[10px] font-black uppercase tracking-wider text-slate-500 mb-2">Digital Transaction Complete</span>
                      <p className="text-[11px] text-slate-600 font-medium">This bill has been successfully registered and archived in the central ledger.</p>
                    </div>
                  </div>
                  ...
                  <button
                    onClick={() => {
                      if (completedBill) handleCancelBill(completedBill.id);
                      setCompletedBill(null);
                      setPosCart({});
                    }}
                    className="w-full bg-red-50 hover:bg-red-100 border border-red-200 text-red-700 font-bold rounded-2xl py-3 text-xs transition-all shadow-sm active:scale-[0.99] cursor-pointer mt-2"
                  >
                    🗑️ Cancel Invoice
                  </button>
                </div>
            ...
            <div className="space-y-5 animate-fade-in text-left">
              ...
              <div className="flex bg-slate-100 p-0.5 sm:p-1 rounded-xl gap-0.5 sm:gap-1.5 shadow-3xs lg:hidden">
                <button
                  type="button"
              ...
              {/* Grid layout containing both columns on desktop */}
              <div className="grid grid-cols-1 lg:grid-cols-12 gap-6 items-start">
                
                {/* Left Column: SALE ITEMS */}
                <div className={`space-y-4 lg:col-span-7 xl:col-span-8 animate-in fade-in duration-200 ${
                  saleSubView === 'items' ? 'block' : 'hidden lg:block'
                }`}>
                  ...
                  {Object.keys(posCart).length > 0 && (
                    <div 
                      onClick={() => setSaleSubView('checkout')}
                      className="bg-slate-900 hover:bg-slate-800 text-white rounded-2xl p-4 flex items-center justify-between shadow-lg border border-slate-800 cursor-pointer transition-all hover:scale-[1.005] group lg:hidden"
                    >
                ...
                <div className={`space-y-4 lg:col-span-5 xl:col-span-4 animate-in fade-in duration-200 text-left ${
                  saleSubView === 'checkout' ? 'block' : 'hidden lg:block'
                }`}>
                  ...
                  {Object.keys(posCart).length === 0 ? (
                    ...
                    <div className="bg-white border border-slate-200/85 rounded-3xl p-5 md:p-6 shadow-sm space-y-5">
                      {/* Store Milk Record Link & Home Delivery Selector */}
                      <div className="bg-emerald-50/50 border border-emerald-100 p-4 rounded-2xl space-y-3">
                        <div className="flex items-center justify-between">
                          <span className="text-[10px] font-black uppercase text-emerald-800 tracking-wider flex items-center gap-1">
                            🥛 Connect Store Milk Record ({store.name.replace("Farmer's Gate - ", "")})
                          </span>
                                  ...
                                  if (found) {
                                    ...
                                    if (found.milkType === 'Both') {
                                      ...
                                      const buffaloItem = {
                                        ...
                                        vegetableName: `Buffalo Milk (User)`,
                                        quantity: 999,
                                      ...
                                      const milkItem = {
                                        ...
                                        vegetableName: `${found.milkType} Milk (User)`,
                                        quantity: 999,
                          ...
                          >
                            ...
                            {(() => {
                              try {
                                const saved = localStorage.getItem('fg_milk_customers');
                                const list = saved ? JSON.parse(saved) : [];
                                // Show only that particular store's users:
                      ...
                      <div className="space-y-4">
                                  ...
                                  onChange={(e) => {
                                    ...
                                    if (val) {
                                      ...
                                      if (foundCust) {
                                        ...
                                        setPosCustomerEmail(foundCust.email || '');
                                        setPosCustomerAddress(foundCust.address || '');
                                        setPosCustomerNotes(foundCust.notes || '');
                                      }
                                    }
                                  }}
                          ...
                          <div className="space-y-1">
                            ...
                            <div className="flex gap-2">
                              ...
                              <a
                                ...
                                target="_blank"
                                referrerPolicy="no-referrer"
                                className="bg-[#25D366] text-white hover:bg-[#128C7E] px-3 py-2 rounded-xl text-[10px] font-black uppercase transition-all shadow-sm flex items-center gap-1.5 cursor-pointer"
                                title="Share cart quote via WhatsApp"
                              >
                        ...
                        {/* Directory-specific Customer Details (Email, Address, Notes) */}
                        <div className="grid grid-cols-1 md:grid-cols-3 gap-3 bg-slate-50/30 p-3 rounded-2xl border border-slate-200/50">
                          <div className="space-y-1 text-left">
                            ...
                            <input
                              ...
                              className="w-full rounded-xl bg-white border border-slate-200 px-3 py-1.5 text-xs font-semibold text-slate-800 placeholder-slate-400 focus:outline-none focus:ring-1 focus:ring-emerald-500"
                            />
                          </div>
                          <div className="space-y-1 text-left">
                            <label className="block text-[9px] font-extrabold uppercase tracking-wide text-slate-400">Address (Optional)</label>
                            <input
                              type="text"
                          ...
                          <div className="space-y-1 text-left">
                            <label className="block text-[9px] font-extrabold uppercase tracking-wide text-slate-400">Customer Notes / Preferences</label>
                            <input
                              type="text"
                              placeholder="prefers organic produce"
                              value={posCustomerNotes}
                              onChange={(e) => setPosCustomerNotes(e.target.value)}
                              className="w-full rounded-xl bg-white border border-slate-200 px-3 py-1.5 text-xs font-semibold text-slate-800 placeholder-slate-400 focus:outline-none focus:ring-1 focus:ring-emerald-500"
                            />
                          </div>
                        </div>
                        ...
                        <div className="flex flex-col sm:flex-row sm:items-center gap-4 bg-emerald-50/30 border border-emerald-100/60 p-3.5 rounded-2xl">
                          <label className="flex items-center gap-2 cursor-pointer text-xs font-extrabold text-slate-700">
                            <input
                              type="checkbox"
                              checked={saveCustomerBill}
                              onChange={(e) => setSaveCustomerBill(e.target.checked)}
                              className="rounded text-emerald-600 focus:ring-emerald-500 w-4 h-4"
                            />
                            📁 Save Customer Bill History
                          </label>
                          <label className="flex items-center gap-2 cursor-pointer text-xs font-extrabold text-slate-700">
                            <input
                              type="checkbox"
                              checked={syncCustomerToDirectory}
                              onChange={(e) => setSyncCustomerToDirectory(e.target.checked)}
                              className="rounded text-emerald-600 focus:ring-emerald-500 w-4 h-4"
                            />
                            👤 Update Details to Customer Directory
                          </label>
                        </div>
                      </div>
                      ...
                      <div className="border-t border-slate-100 pt-5 mt-2 flex flex-col sm:flex-row justify-between items-center gap-4">
                        <div className="text-center sm:text-left space-y-0.5">
                          {(() => {
                            ...
                            if (appliedOffer) {
                              if (appliedOffer.type === 'percentage') {
                                promoDisc = (cartSub * appliedOffer.value) / 100;
                              } else {
                                promoDisc = Math.min(cartSub, appliedOffer.value);
                              }
                            }
                            let manDisc = 0;
                            if (manualDiscountPercent > 0) {
                              manDisc += (cartSub * manualDiscountPercent) / 100;
                            }
                            if (manualDiscountFlat > 0) {
                              manDisc += manualDiscountFlat;
                            }
                            const totalDiscount = Math.min(cartSub, promoDisc + manDisc);
                            const netTotal = cartSub - totalDiscount;
                            ...
                            return (
                              <>
                                ...
                                <div className="text-[10px] font-bold text-slate-400 mb-1">
                                  Subtotal: ₹{cartSub.toFixed(2)} 
                                  {totalDiscount > 0 && <span className="text-rose-500"> | Disc: -₹{totalDiscount.toFixed(2)}</span>}
                                  {isHomeDelivery && <span> | Delivery: {feeApplied === 0 ? <span className="text-emerald-600 font-extrabold uppercase text-[9px]">FREE</span> : `+₹${feeApplied.toFixed(2)}`}</span>}
                                </div>
                        ...
                        <div className="flex gap-2.5 w-full sm:w-auto">
                          <button
                            type="button"
                            onClick={() => {
                              if (confirm('Are you sure you want to cancel this bill? This will clear all items in the current cart.')) {
                                handleClearPosCart();
                              }
                            }}
                            className="flex-1 sm:flex-initial bg-rose-50 hover:bg-rose-100 text-rose-600 font-black rounded-xl px-5 py-3 text-xs transition-colors cursor-pointer border border-rose-100"
                          >
                            CANCEL BILL
                          </button>
                          <button
                    ...
                    {(() => {
                      const filteredBills = storeBills.filter(bill => {
                        ...
                        if (billSearchQuery.trim()) {
                          ...
                          const matchSalesperson = (bill.salespersonName || '').toLowerCase().includes(query);
                          return matchId || matchCustomer || matchStore || matchSalesperson;
                        }
                        return true;
                      });
                      ...
                      return (
                        ...
                        >
                          <div className="space-y-2.5 flex-1 min-w-0">
                            ...
                            <div className="grid grid-cols-2 sm:grid-cols-4 gap-y-1.5 gap-x-4 text-[11px] text-slate-500">
                              <div>
                                <span className="font-extrabold text-slate-400 block uppercase tracking-wider text-[8px]">Customer Name</span>
                                <span className="font-bold text-slate-800 truncate block max-w-[130px]">👤 {bill.customerName || 'Retail Customer'}</span>
                              </div>
                              <div>
                                <span className="font-extrabold text-slate-400 block uppercase tracking-wider text-[8px]">Date & Timestamp</span>
                                <span className="font-medium text-slate-800 block">📅 {bill.date}</span>
                              </div>
                              <div>
                                <span className="font-extrabold text-slate-400 block uppercase tracking-wider text-[8px]">Payment Method</span>
                                <span className="font-bold text-slate-800 block">💳 {bill.paymentMode || 'Cash'}</span>
                              </div>
                              <div>
                                <span className="font-extrabold text-slate-400 block uppercase tracking-wider text-[8px]">Salesperson</span>
                                <span className="font-bold text-emerald-700 block truncate max-w-[120px]">
                                  👔 {bill.salespersonName || 'None'}
                                </span>
                              </div>
                            </div>
                          </div>
      ...
      {activeSubTab === 'requirements' && (
        <div className="grid gap-6 grid-cols-1 lg:grid-cols-3">
          ...
          {/* Create Requirement Checklist & Tools */}
          <div className="space-y-6">
            <div className="rounded-2xl border border-zinc-200 bg-white p-5 shadow-sm space-y-4 h-fit text-left">
              <div className="flex items-center justify-between">
                <div>
                  <h3 className="font-bold text-zinc-900">Order Stock Checklist</h3>
                  <p className="text-[10px] text-zinc-400">Quickly raise requirements from your catalog.</p>
                </div>
                <button 
                  onClick={() => setIsChecklistMode(!isChecklistMode)}
                  className="px-2 py-1 rounded-lg bg-zinc-100 hover:bg-zinc-200 text-[10px] font-bold text-zinc-600 transition-colors cursor-pointer"
                >
                  {isChecklistMode ? "Standard Form" : "Checklist Mode"}
                </button>
              </div>
              ...
              {!isChecklistMode ? (
                ...
                <div className="space-y-4">
                  ...
                  <div className="space-y-3">
                    <div className="relative">
                      ...
                      <input 
                        ...
                        className="w-full rounded-xl border border-zinc-200 pl-8 pr-3 py-2 text-xs focus:border-emerald-500 focus:outline-none focus:ring-1 focus:ring-emerald-500 font-semibold"
                      />
                    </div>

                    <div className="bg-emerald-50/40 p-3 rounded-xl border border-emerald-100 space-y-2">
                      <p className="text-[10px] font-black text-emerald-800 uppercase tracking-tight">Add Custom Item (Not in list)</p>
                      <form onSubmit={handleCustomRequirementSubmit} className="flex gap-1.5">
                        <input 
                          type="text"
                          placeholder="Item Name"
                          value={customRequirementName}
                          onChange={(e) => setCustomRequirementName(e.target.value)}
                          className="flex-1 min-w-0 rounded-lg border border-emerald-200 px-2 py-1.5 text-[11px] focus:outline-none bg-white font-bold"
                        />
                        <input 
                          type="number"
                          placeholder="Qty"
                          value={customRequirementQty}
                          onChange={(e) => setCustomRequirementQty(e.target.value)}
                          className="w-16 rounded-lg border border-emerald-200 px-2 py-1.5 text-[11px] focus:outline-none bg-white font-bold"
                        />
                        <button type="submit" className="bg-emerald-600 text-white rounded-lg px-2 text-[10px] font-black cursor-pointer hover:bg-emerald-700">Add</button>
                      </form>
                    </div>
                  </div>
                  ...
                  <div className="space-y-2.5 max-h-[400px] overflow-y-auto pr-1">
                    {cropCatalogNames
                      .filter(name => !checklistSearchQuery || name.toLowerCase().includes(checklistSearchQuery.toLowerCase()))
                      .map(name => {
                        const existingReq = storeRequirements.find(r => r.vegetableName === name && r.status === 'Pending');
                        const inStock = storeInventory.find(i => i.vegetableName === name);
                        
                        return (
                          <div key={name} className={`p-2.5 rounded-xl border ${existingReq ? 'border-emerald-200 bg-emerald-50/20' : 'border-zinc-100 bg-white'} space-y-2`}>
                            <div className="flex justify-between items-start">
                              <div>
                                <h4 className="text-[11px] font-black text-zinc-800 uppercase leading-tight">{name}</h4>
                                <div className="flex items-center gap-1.5 mt-0.5">
                                  {inStock ? (
                                    <span className={`text-[9px] font-bold ${inStock.quantity <= 10 ? 'text-rose-600' : 'text-zinc-500'}`}>
                                      Stock: {inStock.quantity}kg
                                    </span>
                                  ) : (
                                    <span className="text-[9px] font-bold text-zinc-400">Out of Stock</span>
                                  )}
                                  {existingReq && (
                                    <span className="text-[9px] font-black text-emerald-600 flex items-center gap-0.5">
                                      <span>✅</span> Active: {existingReq.quantity}kg
                                    </span>
                                  )}
                                </div>
                              </div>
                            ...
                            <div className="flex gap-1.5">
                              <div className="relative flex-1">
                                <input 
                                  type="number"
                                  placeholder="Qty (kg)"
                                  value={checklistQuantities[name] || ''}
                                  onChange={(e) => setChecklistQuantities(prev => ({ ...prev, [name]: e.target.value }))}
                                  className="w-full rounded-lg border border-zinc-200 px-2 py-1.5 text-[10px] font-black focus:outline-none focus:border-emerald-500"
                                />
                                <span className="absolute right-2 top-1.5 text-[9px] font-bold text-zinc-400">kg</span>
                              </div>
                              <button 
                                onClick={() => handleChecklistAddRequirement(name)}
                                disabled={!checklistQuantities[name] || parseFloat(checklistQuantities[name]) <= 0}
                                className="bg-zinc-900 text-white rounded-lg px-3 text-[10px] font-black cursor-pointer hover:bg-zinc-800 disabled:opacity-30 transition-all"
                              >
                                {existingReq ? "Update" : "Order"}
                              </button>
                            </div>
                          </div>
                        );
            ...
            <div className="rounded-2xl border border-zinc-200 bg-zinc-50/40 p-5 shadow-sm space-y-3.5 text-left">
              <div>
                ...
                <p className="text-[10px] text-zinc-400 mt-1">Past fulfilled restock requests. Click reuse to raise a new requisition.</p>
              </div>
            ...
            <div className="grid grid-cols-1 lg:grid-cols-12 gap-8">
              ...
              <div className="lg:col-span-7 space-y-6">
                ...
                <div className="flex flex-wrap items-center gap-3 pt-3">
                  ...
                  <div className="flex items-center gap-1.5 text-xs text-slate-500 font-bold bg-slate-100 px-3 py-2 rounded-xl">
                    <span>✨</span> Use the real-time preview on the right for scanning.
                  </div>
                </div>
              ...
              {Object.keys(posCart).length === 0 ? (
                ...
                <div className="space-y-4">
                  ...
                  <div className="space-y-4">
                    ...
                    {/* Customer Database Options */}
                    <div className="flex flex-col sm:flex-row sm:items-center gap-4 bg-emerald-50/30 border border-emerald-100/60 p-3.5 rounded-2xl">
                      <label className="flex items-center gap-2 cursor-pointer text-xs font-extrabold text-slate-700 text-left">
                        <input
                          type="checkbox"
                          checked={saveCustomerBill}
                          onChange={(e) => setSaveCustomerBill(e.target.checked)}
          ...
          <div className="space-y-6 animate-fade-in text-left">
            ...
            <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
              ...
              <div className="lg:col-span-2 space-y-6">
                ...
                {(() => {
                  const salespersonPerformanceOnDate = (() => {
                    const map: { [name: string]: { revenue: number; txCount: Set<string>; itemsCount: number } } = {};
                    
                    salesOnDate.forEach(sale => {
                      const spName = sale.salespersonName || 'No Salesperson / Self-Service';
                      if (!map[spName]) {
                        map[spName] = { revenue: 0, txCount: new Set<string>(), itemsCount: 0 };
                      }
                      map[spName].revenue += sale.totalPrice;
                      map[spName].itemsCount += sale.quantity;
                      if (sale.billId) {
                        map[spName].txCount.add(sale.billId);
                      }
                    });
                    ...
                    return Object.keys(map).map(name => ({
                      ...
                      txCount: map[name].txCount.size,
                      itemsCount: map[name].itemsCount
                  ...
                  return (
                    <div className="bg-white border border-slate-200 rounded-2xl p-5 shadow-sm space-y-4">
                      <div className="flex justify-between items-center border-b border-slate-100 pb-3">
                        <div>
                          <h3 className="text-xs font-black uppercase text-slate-700 tracking-wider">Staff Sales Contribution ({selectedDateStr})</h3>
                          <p className="text-[10px] text-slate-400 mt-0.5">Revenue generated and items sold per salesperson today.</p>
                        </div>
                        <span className="bg-emerald-50 text-emerald-800 border border-emerald-100 text-[9px] font-black uppercase px-2.5 py-0.5 rounded-full">
                          {salespersonPerformanceOnDate.length} Active
                        </span>
                      </div>

                      {salespersonPerformanceOnDate.length === 0 ? (
                        <p className="text-xs text-slate-400 italic py-2 text-center">No sales registered with an attending salesperson for today.</p>
                      ) : (
                        <div className="space-y-2.5">
                          {salespersonPerformanceOnDate.map((sp, idx) => (
                            <div key={sp.name} className="flex items-center justify-between p-3.5 bg-slate-50 border border-slate-200/65 rounded-xl hover:bg-slate-100/40 transition-all">
                              <div className="flex items-center gap-3">
                                <span className={`w-6 h-6 rounded-lg flex items-center justify-center text-xs font-black ${
                                  idx === 0 ? 'bg-amber-100 text-amber-800 border border-amber-200' : 'bg-slate-100 text-slate-600 border border-slate-200'
                                }`}>
                                  {idx + 1}
                                </span>
                                <div>
                                  <span className="font-bold text-slate-800 text-xs block">{sp.name}</span>
                                  <span className="text-[9px] text-slate-400 font-bold uppercase tracking-wider block mt-0.5">
                                    💼 {sp.txCount} bills · {sp.itemsCount.toFixed(1)} kg sold
                                  </span>
                                </div>
                              </div>
                              <div className="text-right">
                                <span className="text-xs font-black text-slate-900 font-mono block">₹{sp.revenue.toFixed(2)}</span>
                                <span className="text-[9px] text-emerald-600 font-extrabold uppercase tracking-wide">
                                  {((sp.revenue / (totalSalesAmount || 1)) * 100).toFixed(0)}% contribution
                                </span>
                              </div>
                            </div>
                          ))}
          ...
          <div className="bg-white rounded-3xl w-full max-w-md shadow-2xl border border-slate-200 overflow-hidden my-8">
            ...
            <div className="bg-slate-900 px-5 py-3.5 flex items-center justify-between text-white print:hidden">
              ...
              <div className="flex items-center gap-2">
                {selectedPreviewBill.status !== 'Cancelled' && (
                  ...
                  <button
                    type="button"
                    onClick={() => handleShareImage(selectedPreviewBill)}
                    className="bg-indigo-600 hover:bg-indigo-500 text-white font-bold text-[10px] px-3 py-1.5 rounded-xl cursor-pointer transition-all flex items-center gap-1"
                  >
                    📄 Image
                  </button>
                  <button
                    type="button"
                    onClick={() => handleSharePDF(selectedPreviewBill)}
                    className="bg-purple-600 hover:bg-purple-500 text-white font-bold text-[10px] px-3 py-1.5 rounded-xl cursor-pointer transition-all flex items-center gap-1"
                  >
                    📕 PDF
                  </button>
                  <button
                    type="button"
                    onClick={() => setPrintPreviewBill(selectedPreviewBill)}
            ...
            <div id="print-receipt-area" className="p-8 space-y-6 font-mono text-xs text-slate-800 bg-white relative overflow-hidden">
              ...
              <div className="text-center space-y-1.5">
                <h3 className="text-sm font-black tracking-tighter uppercase text-slate-800">Farmer's Gate</h3>
      ...
      {printPreviewBill && (
        <div className="fixed inset-0 z-[3000] flex flex-col bg-slate-900/95 backdrop-blur-sm p-4 sm:p-6 overflow-y-auto print:bg-white print:p-0">
          
          {/* Top Control Header Bar (Hidden on actual print) */}
          <div className="max-w-md w-full mx-auto bg-slate-850 border border-slate-700/60 rounded-2xl p-4 mb-6 flex items-center justify-between shadow-xl print:hidden text-white animate-in fade-in slide-in-from-top-4 duration-300">
            <div className="flex items-center gap-2.5">
              <span className="bg-emerald-500/10 text-emerald-400 border border-emerald-500/20 p-2 rounded-xl text-sm">
                🖨️
              </span>
              <div className="text-left">
                <h4 className="text-xs font-black uppercase tracking-wider text-slate-200">Print Preview</h4>
                <p className="text-[10px] text-slate-400 font-bold">Physical Thermal Receipt Layout</p>
              </div>
            </div>
            ...
            <div className="flex items-center gap-2">
              <button
                ...
                className="bg-emerald-600 hover:bg-emerald-500 active:scale-[0.98] text-white font-black text-xs px-4 py-2 rounded-xl cursor-pointer transition-all flex items-center gap-1.5 shadow-lg shadow-emerald-900/25"
              >
                🖨️ Print Now
              </button>
              ...
              <button
                type="button"
                onClick={() => setPrintPreviewBill(null)}
                className="bg-slate-800 hover:bg-slate-700 active:scale-[0.98] text-slate-300 font-extrabold text-xs px-3.5 py-2 rounded-xl cursor-pointer transition-all border border-slate-700"
              >
                Close
              </button>
            </div>
          ...
          {/* Simulated Physical Thermal Paper Receipt */}
          <div className="max-w-sm w-full mx-auto bg-white rounded-lg shadow-2xl border border-slate-200 overflow-hidden relative print:shadow-none print:border-none print:my-0 print:p-0 print:rounded-none animate-in fade-in zoom-in-95 duration-300">
            
            {/* Top Cutout Visual Indicator (Not printed) */}
            <div className="absolute top-0 inset-x-0 h-1.5 bg-repeat-x bg-[linear-gradient(to_right,transparent_50%,#cbd5e1_50%)] bg-[size:10px_6px] print:hidden"></div>

            <div id="print-receipt-area" className="p-8 space-y-6 font-mono text-xs text-slate-800 bg-white relative overflow-hidden print:p-4 print:text-[10px]">
              
              {/* Receipt Header */}
              <div className="text-center space-y-1.5">
                <h3 className="text-base font-black tracking-tighter uppercase text-slate-900">Farmer's Gate</h3>
                <p className="text-[9px] font-black uppercase text-slate-500 tracking-widest">
                  📍 {printPreviewBill.storeName || store.name} ({printPreviewBill.storeAbbreviation || getStoreAbbreviation(printPreviewBill.storeName || store.name)})
                </p>
                {printPreviewBill.storeId && (
                  <p className="text-[9px] font-mono text-slate-400 uppercase">
                    Store ID: {printPreviewBill.storeId}
                  </p>
                )}
                <div className="inline-block px-3 py-0.5 rounded-full text-[9px] font-black uppercase bg-slate-100 border border-slate-200 mt-1">
                  Invoice No: #{printPreviewBill.id}
                </div>
              </div>

              {/* Status Banner */}
              {printPreviewBill.status === 'Cancelled' ? (
                <div className="bg-rose-50 border border-rose-200 rounded-xl p-2.5 text-center text-rose-700 font-bold uppercase tracking-wider text-[10px]">
                  🚨 Cancelled Transaction 🚨
                </div>
              ) : (
                <div className="border-y border-dashed border-slate-200 py-2.5 text-[10px] text-slate-500 flex justify-between font-medium">
                  <span>📅 Date: {printPreviewBill.date}</span>
                  <span className="font-bold text-emerald-700">🟢 Paid Active</span>
                </div>
              )}
              ...
              <div className="bg-slate-50/80 rounded-xl p-2.5 border border-slate-150 text-[10px] space-y-1 text-left">
                ...
                <div className="flex justify-between font-black text-slate-800 text-[11px]">
                  <span className="truncate max-w-[150px]">👤 {printPreviewBill.customerName || 'Retail Customer'}</span>
                  <span>{printPreviewBill.isHomeDelivery ? '🚚 HOME' : '🏪 STORE'}</span>
                </div>
                {printPreviewBill.salespersonName && (
                  <div className="flex justify-between text-slate-500 font-bold text-[9px] mt-1 pt-1 border-t border-slate-100">
                    <span>ATTENDING SALESPERSON</span>
                    <span className="font-extrabold uppercase">{printPreviewBill.salespersonName}</span>
                  </div>
                )}
                {printPreviewBill.isHomeDelivery && (
                  <div className="flex justify-between text-emerald-800 font-extrabold text-[9px] mt-1 pt-1 border-t border-slate-100">
                    <span>TYPE: HOME DELIVERY</span>
                    <span>Fee: ₹{printPreviewBill.deliveryCharges || 0}</span>
                  </div>
                )}
              </div>
              ...
              <div className="space-y-2">
                ...
                <div className="space-y-3 pt-1">
                  {printPreviewBill.items && printPreviewBill.items.map((it: any, idx: number) => {
                    const unitLabel = it.unit || 'kg';
                    const baseLabel = unitLabel === 'g' ? 'kg' : 'unit';
                    return (
                      <div key={idx} className="flex justify-between items-start text-xs text-left">
              ...
              {/* Totals Breakdown */}
              <div className="border-t border-dashed border-slate-300 pt-4 space-y-1.5 font-medium text-slate-600 text-left">
                {printPreviewBill.subtotal !== undefined && (
                  <div className="flex justify-between">
                    <span>Subtotal</span>
                    <span className="font-mono">₹{printPreviewBill.subtotal.toFixed(2)}</span>
                  </div>
                )}
                {printPreviewBill.discount !== undefined && printPreviewBill.discount > 0 && (
                  <div className="flex justify-between text-red-500 font-bold">
                    <span>Promo Discount</span>
                    <span className="font-mono">-₹{printPreviewBill.discount.toFixed(2)}</span>
                  </div>
                )}
                {printPreviewBill.isHomeDelivery && (
                  <div className="flex justify-between text-emerald-700 font-bold">
                    <span>Delivery Charges</span>
                    <span className="font-mono">+₹{(printPreviewBill.deliveryCharges || 0).toFixed(2)}</span>
                  </div>
                )}
                <div className="flex justify-between items-baseline pt-2 border-t border-slate-200 text-slate-900 font-black text-sm">
                  <span>TOTAL AMOUNT</span>
                  <span className="font-mono text-base text-emerald-700 font-extrabold">₹{printPreviewBill.totalAmount.toFixed(2)}</span>
                </div>
              </div>
              ...
              <div className="bg-slate-50 border border-slate-200 rounded-xl p-3 flex justify-between items-center text-[10px] uppercase font-bold text-slate-500">
                <span>Payment Mode:</span>
                <span className="text-slate-800">{printPreviewBill.paymentMode || 'Cash'}</span>
              </div>
              ...
              <div className="flex flex-col items-center justify-center space-y-1 py-1">
                ...
                <span className="text-[8px] tracking-widest text-slate-400 font-mono">{printPreviewBill.id}</span>
              </div>
            ...
            {/* Extra style tag to target print media specifically for this preview container */}
            <style dangerouslySetInnerHTML={{__html: `
              @media print {
                body * {
                  visibility: hidden !important;
