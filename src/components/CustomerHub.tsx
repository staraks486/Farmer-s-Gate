...
export default function CustomerHub({ changePortal, appVersion }: { changePortal?: (portal: 'customer' | 'partner' | 'management' | 'executive' | 'store_pos') => void, appVersion?: string }) {
  const { products, cpanelSettings, loading: dataLoading, stores, staff } = useData();
  ...
  // Selected Physical Store and Custom WhatsApp Number states
  ...
  useEffect(() => {
    const handleScroll = () => {
      ...
      if (currentScrollY > 80) {
        ...
        setShowHeader(true); // near top, always show
      }
  ...
  // Compute currently selected store or fallback to first (filtering only active stores for shoppers)
  const activeStores = (stores || []).filter(s => s.isActive);
  const selectedStore = (activeStores && activeStores.find(s => s.id === selectedStoreId)) || (activeStores && activeStores[0]) || null;
  const selectedStoreManager = staff?.find(s => s.assignedStoreId === selectedStore?.id && s.role === 'Manager' && s.isActive) || staff?.find(s => s.role === 'Manager' && s.isActive) || null;
  ...
  // Initial seeding (optional, handled by App.tsx or DataContext indirectly)
  useEffect(() => {
    seedProductsIfNeeded();
  }, []);

  // Synchronize and seed default store and whatsapp number from active context stores
  useEffect(() => {
    const activeStoresList = (stores || []).filter(s => s.isActive);
    if (activeStoresList && activeStoresList.length > 0) {
      const defaultStore = activeStoresList[0];
      if (!selectedStoreId) {
        ...
        if (localStoresStr) {
          try {
            ...
            if (localStores && localStores.length > 0) {
              const matched = activeStoresList.find(s => s.id === localStores[0].id);
              if (matched) {
                setSelectedStoreId(matched.id);
              }
              localWhatsapp = localStores[0].whatsappNumber;
            }
          } catch (e) {
            console.error("Error reading fg_stores:", e);
          }
        }
        
        const targetNum = localWhatsapp || defaultStore.whatsappNumber || '';
        setCustomWhatsappNumber(targetNum);
  ...
  // Clean, synchronize and cache only active stores in local storage for use across sibling pages
  useEffect(() => {
    const activeStoresList = (stores || []).filter(s => s.isActive);
    if (activeStoresList.length > 0) {
      localStorage.setItem('fg_cached_stores', JSON.stringify(activeStoresList));
      const localStoresStr = localStorage.getItem('fg_stores');
      if (localStoresStr) {
        try {
          const localStores = JSON.parse(localStoresStr);
          const hasInactive = localStores.some((ls: any) => !activeStoresList.some((as: any) => as.id === ls.id));
          if (hasInactive) {
            localStorage.setItem('fg_stores', JSON.stringify([{
              id: activeStoresList[0].id,
              name: activeStoresList[0].name,
              location: activeStoresList[0].location,
              whatsappNumber: activeStoresList[0].whatsappNumber || ''
            }]));
          }
        } catch (e) {}
      }
  ...
  const handleAuthSubmit = async (e: React.FormEvent) => {
    ...
    if (authMode === 'login') {
      // Direct Email Login! Skip OTP entirely!
      setLoadingAuth(true);
      try {
        try {
          const credential = await signInWithEmailAndPassword(auth, cleanEmail, authPassword);
          if (credential.user) {
            const storedName = localStorage.getItem(`fg_name_${credential.user.uid}`);
            const storedPhone = localStorage.getItem(`fg_phone_${credential.user.uid}`);
            const storedAddress = localStorage.getItem(`fg_address_${credential.user.uid}`);
            
            if (storedName) setAuthName(storedName);
            if (storedPhone) setAuthPhone(storedPhone);
            if (storedAddress) setCustomAddress(storedAddress);
          }
        } catch (loginErr: any) {
          ...
          if ((isDemoShopper || isDemoRider) && authPassword === 'farmersgate123') {
            try {
              const credential = await createUserWithEmailAndPassword(auth, cleanEmail, authPassword);
              if (credential.user) {
              ...
              setAuthName(isDemoRider ? 'Live Partner Rider' : 'Gold Shopper');
              setAuthPhone(isDemoRider ? '+91 90000 00000' : '+91 95000 12345');
              setCustomAddress('FarmersGate HQ Dispatch Hub, Block C, Bengaluru');
            }
          } else {
            ...
            const mockUser = {
              uid: `mock_user_${Date.now()}`,
              email: cleanEmail,
              displayName: authName || cleanEmail.split('@')[0],
              emailVerified: true
            };
            localStorage.setItem('fg_mock_user', JSON.stringify(mockUser));
            localStorage.setItem(`fg_name_${mockUser.uid}`, authName || cleanEmail.split('@')[0]);
            localStorage.setItem(`fg_phone_${mockUser.uid}`, authPhone || '+91 95000 12345');
            localStorage.setItem(`fg_address_${mockUser.uid}`, authAddress || '402, Green Meadows, Bangalore');
            
            setUser(mockUser as any);
            setAuthName(authName || cleanEmail.split('@')[0]);
            setAuthPhone(authPhone || '+91 95000 12345');
            setCustomAddress(authAddress || '402, Green Meadows, Bangalore');
          }
        }
    ...
    // Generate a fixed OTP code for registration mode only
    const code = "123456";
    setExpectedOtp(code);
    setEnteredOtp('');
    setOtpError('');
  ...
  const handleSendOrderWhatsApp = (order: FirebaseOrder) => {
    try {
      const targetNum = customWhatsappNumber || (selectedStore && selectedStore.whatsappNumber) || "";
      const storeName = (selectedStore && selectedStore.name) || "Farmer's Gate";
      const storeLocation = (selectedStore && selectedStore.location) || "FarmersGate HQ Dispatch Hub, Bangalore";
      const storeMgr = staff?.find(s => s.assignedStoreId === selectedStore?.id && s.role === 'Manager' && s.isActive) || staff?.find(s => s.role === 'Manager' && s.isActive);
      const storeManagerName = storeMgr ? storeMgr.name : "Ramesh Kumar";
      const storeManagerPhone = storeMgr ? formatPhoneWithCountryCode(storeMgr.phone) : "+91 98765 43210";

      const items = order.items.map(it => ({
        vegetableName: it.vegetableName,
        quantity: it.quantity,
        pricePerKg: it.pricePerKg,
        totalPrice: it.totalPrice || (it.pricePerKg * it.quantity),
        emoji: it.emoji,
        unit: (it as any).unit || 'Kg'
      }));
      ...
      sendWhatsAppOrder({
        ...
        walletDebited: order.walletDebited || 0,
        deliveryFee: order.finalPaid - (order.totalAmount - (order.discount || 0) - (order.walletDebited || 0)),
      ...
      console.error("Error triggering WhatsApp share:", err);
      alert("Failed to send details via WhatsApp.");
    }
  };
  ...
  const getWhatsAppOrderUrl = () => {
    try {
      const num = customWhatsappNumber || (selectedStore && selectedStore.whatsappNumber) || "";
      const storeName = (selectedStore && selectedStore.name) || "Farmer's Gate";
      const storeLocation = (selectedStore && selectedStore.location) || "FarmersGate HQ Dispatch Hub, Bangalore";
      const storeMgr = staff?.find(s => s.assignedStoreId === selectedStore?.id && s.role === 'Manager' && s.isActive) || staff?.find(s => s.role === 'Manager' && s.isActive);
      const storeManagerName = storeMgr ? storeMgr.name : "Ramesh Kumar";
      ...
      const items = cartItemsList.map(({ item, quantity }) => ({
      ...
      return getWhatsAppLink(generateWhatsAppOrderMessage({
        customerName: authName || 'Guest Shopper',
        customerPhone: authPhone || 'N/A',
        customerAddress: customAddress || authAddress || 'Patiala Region',
        items,
  ...
  const handlePlaceOrder = async () => {
    ...
    try {
      const orderNum = generateId('ORD').slice(0, 10).toUpperCase();
  ...
  return (
    <div className="min-h-screen bg-[#FAF9FD] flex flex-col font-sans select-none antialiased text-slate-800 pb-20 relative">
      ...
      <div className={`bg-emerald-600 text-white text-[10px] font-black text-center py-1.5 uppercase tracking-widest hidden sm:flex items-center justify-center gap-1.5 transition-all duration-300 ${showHeader ? 'max-h-12 opacity-100' : 'max-h-0 opacity-0 py-0 overflow-hidden'}`}>
      ...
      <div className={`bg-gradient-to-b from-[#F3E8FF]/90 via-[#F3E8FF]/40 to-white/20 border-b border-purple-100/50 px-4 py-3 shrink-0 transition-all duration-300 ${showHeader ? 'max-h-[200px] opacity-100 py-3 border-b border-purple-100/50' : 'max-h-0 opacity-0 py-0 border-b-0 overflow-hidden'}`}>
        <div className="max-w-7xl mx-auto flex flex-col md:flex-row justify-between items-stretch md:items-center gap-3">
          ...
          <div className="flex items-center gap-3">
            ...
            <div>
              ...
              <span className="text-[10.5px] font-bold text-slate-500 uppercase tracking-tight hidden sm:flex items-center gap-1">
        ...
        {activeTab === 'shop' && (
          <>
            ...
            <div className="lg:col-span-3 space-y-5">
              ...
              {/* Premium Welcome Message Banner */}
              <div id="shopper-welcome-banner" className="bg-gradient-to-r from-emerald-50 to-teal-50 border border-emerald-100/70 p-5 rounded-3xl shadow-3xs flex flex-col md:flex-row justify-between items-start md:items-center gap-4 animate-fade-in">
                <div className="space-y-1">
                  <h3 className="text-sm font-black text-emerald-950 uppercase tracking-tight flex items-center gap-1.5">
                    <span>👋</span> {user ? `Welcome back, ${authName || 'Valued Shopper'}!` : 'Welcome to FarmersGate Express!'}
                  </h3>
                  <p className="text-[11px] text-slate-600 font-medium leading-relaxed">
                    {user 
                      ? 'Sourcing farm-fresh organically harvested greens, sweet fruits, and essentials directly to your Bangalore doorstep.'
                      : 'Sign in now to unlock premium organic discounts, keep track of order history, and access express local delivery.'}
                  </p>
                </div>
                {!user && (
                  <button 
                    ...
                    className="px-4 py-2 bg-emerald-700 hover:bg-emerald-800 text-white text-[10px] font-black uppercase rounded-2xl cursor-pointer shadow-xs whitespace-nowrap"
                  >
                    🔐 Sign In Account
                  </button>
                )}
              ...
              <div id="instant-basket-card" className="bg-white border border-slate-200 rounded-3xl p-4 sticky top-4 shadow-sm space-y-4 transition-all duration-300">
                ...
                {cartItemsList.length === 0 ? (
                  ...
                  <div className="space-y-4">
                    ...
                    {/* Dispatch Hub physical location & custom WhatsApp contact options */}
                    <div className="bg-emerald-50/70 border border-emerald-100 p-3.5 rounded-2xl space-y-2.5">
                      <div className="flex justify-between items-center">
                        <span className="text-[9px] font-black text-emerald-800 uppercase tracking-widest block">🏪 Dispatch Hub Location</span>
                        <span className="text-[8.5px] bg-emerald-600 text-white font-black px-1.5 py-0.5 rounded-md uppercase tracking-wider scale-95 origin-right">
                          Active Branch
                        </span>
                      </div>
                      ...
                      <div className="space-y-2">
                        {activeStores && activeStores.length > 0 ? (
                          <div className="space-y-1">
                            <label className="text-[8px] font-bold text-slate-400 uppercase block">Select Store</label>
                            <select
                              id="shopper-store-location-select"
                              value={selectedStoreId}
                              onChange={(e) => {
                                const sId = e.target.value;
                                setSelectedStoreId(sId);
                                const found = activeStores.find(s => s.id === sId);
                                if (found) {
                                  // Update localStorage fg_stores to synchronize with user's choices
                                  const localStores = [{
                                    id: found.id,
                                    name: found.name,
                                  ...
                                  localStorage.setItem('fg_stores', JSON.stringify(localStores));
                                  setCustomWhatsappNumber(found.whatsappNumber || '');
                                }
                              }}
                              className="w-full text-[11px] font-black px-2 py-1.5 bg-white border border-slate-200 rounded-xl focus:outline-none focus:ring-2 focus:ring-emerald-500 cursor-pointer text-slate-800"
                            >
                          ...
                          <div className="text-[10px] font-bold text-slate-400 animate-pulse flex items-center gap-1.5 py-1">
                            <span>🏪</span> Loading Dispatch Locations...
                          </div>
                        )}

                        <div className="bg-white/90 p-2.5 border border-emerald-100 rounded-xl space-y-1.5 shadow-3xs">
                          <div className="flex items-start gap-1.5 text-[10px]">
                            <span className="text-slate-400 text-xs mt-0.5">📍</span>
                            <div className="space-y-0.5">
                              <span className="text-[8px] font-black text-slate-400 uppercase block">Store Address & Location</span>
                              <span className="font-extrabold text-slate-700 leading-normal block">
                                {selectedStore?.location || 'FarmersGate HQ Dispatch Hub, Bangalore'}
                              </span>
                            </div>
                          </div>

                          <div className="flex items-start gap-1.5 text-[10px] border-t border-emerald-50/50 pt-1.5">
                            <span className="text-slate-400 text-xs mt-0.5">👤</span>
                            <div className="space-y-0.5">
                              <span className="text-[8px] font-black text-slate-400 uppercase block">Store Manager Contact</span>
                              <span className="font-extrabold text-slate-700 leading-normal block">
                                {selectedStoreManager 
                                  ? `${selectedStoreManager.name} (${formatPhoneWithCountryCode(selectedStoreManager.phone)})` 
                          ...
                          <div className="flex items-start gap-1.5 text-[10px] border-t border-emerald-50/50 pt-1.5">
                            ...
                            <div className="space-y-0.5">
                              <span className="text-[8px] font-black text-slate-400 uppercase block">WhatsApp Line</span>
                              <span className="font-black text-emerald-700 block font-mono">
                                {selectedStore?.whatsappNumber || '+91 95000 12345'}
                              </span>
                            </div>
                          </div>
                        </div>
                        ...
                        <div className="space-y-1 pt-1 border-t border-emerald-100/50">
                          <label className="text-[8px] font-bold text-slate-400 uppercase block">
                            Custom Mobile Number for WhatsApp Order
                          </label>
                          <input
                            id="shopper-custom-whatsapp-input"
                            type="text"
                            placeholder="e.g. +91 90000 12345"
                            value={customWhatsappNumber}
                            onChange={(e) => {
                              const val = e.target.value;
                              setCustomWhatsappNumber(val);

                              // Override the target phone number stored in localStorage fg_stores
                              const localStoresStr = localStorage.getItem('fg_stores');
                              const localStores = localStoresStr ? JSON.parse(localStoresStr) : [];
                              if (localStores.length > 0) {
                                localStores[0].whatsappNumber = val;
                                localStorage.setItem('fg_stores', JSON.stringify(localStores));
                              } else {
                                localStorage.setItem('fg_stores', JSON.stringify([{
                                  id: selectedStore?.id || 'default_store',
                                  name: selectedStore?.name || "Farmer's Gate",
                                  location: selectedStore?.location || 'FarmersGate HQ',
                                  whatsappNumber: val
                                }]));
                              }
                            }}
                            className="w-full text-[10.5px] font-black px-2.5 py-1.5 bg-white border border-slate-200 rounded-xl focus:outline-none focus:ring-2 focus:ring-emerald-500 font-mono text-slate-800"
                          />
                          <p className="text-[7.5px] text-slate-400 leading-normal font-medium mt-0.5">
                            * Enter custom phone number to route dispatch orders to a specific local rider or custom partner WhatsApp line.
                          </p>
                        </div>
                      </div>
                    </div>
                ...
                <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
                  ...
                  <div className="lg:col-span-1 space-y-4">
                    <div className="bg-slate-50 border border-slate-200 rounded-2xl p-4.5 space-y-4">
                      ...
                      {/* Order Complete Status Info */}
                      <div className="bg-slate-50 border border-slate-200/60 p-3 rounded-xl space-y-1">
                        <span className="text-[9px] font-black text-slate-400 uppercase tracking-wider block">Digital Receipt Status</span>
                        <p className="text-[10px] text-slate-600 font-bold leading-normal">
                          This transaction has been successfully recorded in your wallet. You can view full history in the "Transactions" tab.
                        </p>
                      </div>
                    ...
                    {/* WhatsApp share/send details */}
                    <button
                      type="button"
                      onClick={() => handleSendOrderWhatsApp(selectedOrder)}
                      className="w-full py-3 bg-emerald-600 hover:bg-emerald-700 text-white font-black rounded-2xl text-xs uppercase tracking-wider flex items-center justify-center gap-2 shadow-xs transition-all cursor-pointer"
                    >
                    ...
                    >
                      ...
                      <div className="flex flex-wrap gap-2 pt-2 border-t border-slate-100">
                        ...
                        <button
                          ...
                          className="px-3 py-1.5 bg-white border border-slate-200 hover:border-emerald-500/20 hover:text-emerald-600 text-slate-700 rounded-lg text-[10px] font-bold uppercase transition-all cursor-pointer flex items-center gap-1"
                        >
                          💬 WhatsApp Receipt
                        </button>
                        ...
                        <div className="flex justify-between items-center border-t border-slate-100/80 pt-2.5">
                          ...
                          <div className="flex items-center gap-2">
                            <button
                              type="button"
                              onClick={() => handleSendOrderWhatsApp(order)}
                              className="bg-emerald-600 hover:bg-emerald-700 text-white font-extrabold text-[10px] uppercase tracking-wider px-3 py-2 rounded-lg cursor-pointer transition-all flex items-center gap-1 shadow-3xs"
                            >
                              <span>💬</span> WhatsApp
                            </button>
              ...
              {verificationStep ? (
                ...
                <>
                  <div className="text-center space-y-2">
                    <h3 className="font-extrabold text-emerald-800 text-xl tracking-tight uppercase">
                      {cpanelSettings?.appName || "Farmer's Gate"}
                    </h3>
      ...
      {cartItemsList.length > 0 && (
        <div className="fixed bottom-16 left-4 right-4 z-50 md:hidden animate-fade-in-up">
          <button
            onClick={() => {
      ...
      <div className="fixed bottom-0 left-0 right-0 z-50 bg-white border-t border-slate-200 shadow-[0_-4px_12px_rgba(0,0,0,0.05)] px-6 py-2 flex justify-around items-center md:hidden">
        ...
        <button
          ...
          className="flex flex-col items-center gap-1 cursor-pointer transition text-slate-500 hover:text-slate-700 relative"
          id="mobile-voice-assistant-tab"
        >
          <div className="relative">
            <Mic className="h-4.5 w-4.5 text-slate-500" />
